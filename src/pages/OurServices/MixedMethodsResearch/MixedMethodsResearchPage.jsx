...
import styles from './MixedMethodsResearchPage.module.css';

export default function MixedMethodsResearchPage() {
  return (
    <div className={styles.container}>
      <SEO
        ...
        keywords={"Mixed Methods Research, research-methods, data analytics services, Scape Data Solutions"}
      />
      <section style={{ padding: '4rem 0 5rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.6rem)', marginBottom: '.5rem' }}>{"Mixed Methods Research"}</h1>
        <p style={{ fontSize: '1.15rem', color: 'var(--color-text-secondary, #64748b)', marginBottom: '1.5rem' }}>{"Rigor you can defend"}</p>
        <p style={{ maxWidth: '680px', lineHeight: 1.7, marginBottom: '2rem' }}>{"Rigorous Mixed Methods Research support, from design through interpretation, for dissertations and peer-reviewed research."}</p>
        ...
        <p>
          <Link to="/contact" style={{ display: 'inline-block', padding: '.75rem 1.6rem', borderRadius: '999px', background: 'var(--color-text, #0a0a0a)', color: 'var(--color-bg, #fff)', textDecoration: 'none', fontWeight: 500 }}>
            Talk to our team about {"Mixed Methods Research"}
          </Link>
        </p>
      </section>
    </div>
