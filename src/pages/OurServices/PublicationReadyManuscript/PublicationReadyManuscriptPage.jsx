...
import styles from './PublicationReadyManuscriptPage.module.css';

export default function PublicationReadyManuscriptPage() {
  return (
    <div className={styles.container}>
      ...
      <section style={{ padding: '4rem 0 5rem' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.6rem)', marginBottom: '.5rem' }}>{"Publication Ready Manuscript"}</h1>
        <p style={{ fontSize: '1.15rem', color: 'var(--color-text-secondary, #64748b)', marginBottom: '1.5rem' }}>{"Publication-ready, every time"}</p>
        <p style={{ maxWidth: '680px', lineHeight: 1.7, marginBottom: '2rem' }}>{"Professional Publication Ready Manuscript that strengthens your manuscript for submission, review, and publication."}</p>
        ...
        <p>
          <Link to="/contact" style={{ display: 'inline-block', padding: '.75rem 1.6rem', borderRadius: '999px', background: 'var(--color-text, #0a0a0a)', color: 'var(--color-bg, #fff)', textDecoration: 'none', fontWeight: 500 }}>
            Talk to our team about {"Publication Ready Manuscript"}
          </Link>
        </p>
      </section>
    </div>
